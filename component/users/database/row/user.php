...
class DatabaseRowUser extends Library\DatabaseRowTable
{
    ...
    public function getRole()
    {
        if (!$this->_role)
        {
            //@TODO : Temporarily using  Library\ObjectManager::get since User object is not yet properly set on session when
            // getting it with JFactory::getUser.
            $this->_role =  Library\ObjectManager::get('com:users.model.roles')->id($this->role_id)->getRow();
    ...
    public function getGroups()
    {
        if(is_null($this->_groups))
        {
            if(!$this->guest)
            {
                $this->_groups =  Library\ObjectManager::get('com:users.database.table.groups_users')
