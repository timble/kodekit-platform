...
new edButton('ed_strong', 'b', '<strong>', '</strong>', 'b');

edButtons[edButtons.length] =
new edButton('ed_em', 'i', '<em>', '</em>', 'i');

edButtons[edButtons.length] =
new edButton('ed_link', 'link', '', '</a>', 'a'); // special case
edButtons[edButtons.length] =
new edButton('ed_block', 'b-quote', '\n\n<blockquote>', '</blockquote>\n\n', 'q');


edButtons[edButtons.length] =
new edButton('ed_del', 'del', '<del datetime="' + datetime + '">', '</del>', 'd');

edButtons[edButtons.length] =
new edButton('ed_ins', 'ins', '<ins datetime="' + datetime + '">', '</ins>', 's');

edButtons[edButtons.length] =
new edButton('ed_img', 'img', '', '', 'm', -1); // special case
edButtons[edButtons.length] =
new edButton('ed_ul', 'ul', '<ul>\n', '</ul>\n\n', 'u');

edButtons[edButtons.length] =
new edButton('ed_ol', 'ol', '<ol>\n', '</ol>\n\n', 'o');

edButtons[edButtons.length] =
new edButton('ed_li', 'li', '\t<li>', '</li>\n', 'l');

edButtons[edButtons.length] =
new edButton('ed_code', 'code', '<code>', '</code>', 'c');

edButtons[edButtons.length] =
new edButton('ed_more', 'more', '<hr id="system-readmore" />', '', 't', -1);
/*
edButtons[edButtons.length] =
...
function edQuickLink(i, thisSelect) {
    if (i > -1) {
        ...
        tempStr = '<a href="' + edLinks[i].URL + '"' + newWin + '>' + edLinks[i].display + '</a>';
...
function edInsertTag(myField, i) {
    var codemirror = switchEditors.codemirror,
        selection = codemirror.selection();

    if (selection) {
        codemirror.replaceSelection(edButtons[i].tagStart + selection + edButtons[i].tagEnd);
    } else {
        if (!edCheckOpenTags(i) || edButtons[i].tagEnd == '') {
            codemirror.replaceSelection(edButtons[i].tagStart + selection);
            edAddTag(i);
        } else {
            codemirror.replaceSelection(edButtons[i].tagEnd + selection);
            edRemoveTag(i);
        }
    }
}
...
function edInsertContent(myField, myValue) {
    switchEditors.codemirror.replaceSelection(myValue);
}
...
var QTags = function (name, id, container, disabled) {
    ...
    if (!t.Canvas || !cont) return;

    disabled = (typeof disabled != 'undefined') ? ',' + disabled + ',' : '';

    t.edShowButton = function (button, i) {
        if (disabled && (disabled.indexOf(',' + button.display + ',') != -1)) return '';
        else if (button.id == name + '_img') return '<input type="button" id="' + button.id + '" accesskey="' + button.access + '" class="ed_button" onclick="edInsertImage(' + name + '.Canvas);" value="' + button.display + '" />';
        else if (button.id == name + '_link') return '<input type="button" id="' + button.id + '" accesskey="' + button.access + '" class="ed_button" onclick="' + name + '.edInsertLink(' + i + ');" value="' + button.display + '" />';
        else return '<input type="button" id="' + button.id + '" accesskey="' + button.access + '" class="ed_button" onclick="' + name + '.edInsertTag(' + i + ');" value="' + button.display + '" />';
    };
    ...
    this.edQuickLink = function (i, thisSelect) {
        if (i > -1) {
            ...
            tempStr = '<a href="' + Links[i].URL + '"' + newWin + '>' + Links[i].display + '</a>';
            thisSelect.selectedIndex = 0;
    ...
    t.edInsertTag = function (i) {
        ...
        if (document.selection) {
            ...
            var startPos = t.Canvas.selectionStart,
                endPos = t.Canvas.selectionEnd,
                cursorPos = endPos,
                scrollTop = t.Canvas.scrollTop;

            if (startPos != endPos) {
                t.Canvas.value = t.Canvas.value.substring(0, startPos) + t.Buttons[i].tagStart + t.Canvas.value.substring(startPos, endPos) + t.Buttons[i].tagEnd + t.Canvas.value.substring(endPos, t.Canvas.value.length);
                cursorPos += t.Buttons[i].tagStart.length + t.Buttons[i].tagEnd.length;
            } else {
                if (!t.edCheckOpenTags(i) || t.Buttons[i].tagEnd == '') {
                    t.Canvas.value = t.Canvas.value.substring(0, startPos) + t.Buttons[i].tagStart + t.Canvas.value.substring(endPos, t.Canvas.value.length);
                    t.edAddTag(i);
                    cursorPos = startPos + t.Buttons[i].tagStart.length;
                } else {
                    t.Canvas.value = t.Canvas.value.substring(0, startPos) + t.Buttons[i].tagEnd + t.Canvas.value.substring(endPos, t.Canvas.value.length);
                    t.edRemoveTag(i);
    ...
    t.Buttons[t.Buttons.length] = new edButton(name + '_more', 'more', '<hr id="system-readmore" />', '', 't', -1);
