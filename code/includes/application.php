	...
	*/
	...
	function route($uri = null)
	{
		...
		if(KRequest::type() != 'AJAX')
		{
		    ...
		    {
		        $page = JFactory::getApplication()->getPages()->getHome();
		        $route = JRoute::_('index.php?Itemid='.$page->id);
	...
	function dispatch()
	{
		...
		{
		    if(!JFactory::getApplication()->getPages()->isAuthorized(JRequest::getInt('Itemid'), $user->get('aid')))
		    {
			    if (!$user->get('aid')) {
				    $option = 'com_users';
			    } else {
	...
	function &getParams($option = null)
	{
		...
		if (!isset($params[$hash]))
		{
			...
			if (is_object($page))
			{
				$params[$hash]->merge(new JParameter($page->params));
				$title = $page->name;
	...
	function &getRouter()
	{
		...
		$options['mode'] = '1'; //$config->getValue('config.sef');
	 ...
	 * request. If not found it falls back on the default.
	 * 
	 ...
	 * @since	Nooku Server 0.7
	 */
    protected function _loadSite($site = null)
	{
	    if(!$site)
	    {
	        // Check URL host
