...
var Chosen = new Class({
	...
	result_do_highlight: function(el){
		...
		if (el){
			...
			high_top = this.result_highlight.getPosition(this.search_results).y + this.search_results.getParent().getScroll().y;
...
Element.implement({
	get_side_border_padding: function(){
		...
		var notNull = new Hash(styles).filter(function(value){
			return (typeof(value) == 'string');
		});
		var mapped = notNull.map(function(s){ return s.toInt();});
		var array = mapped.getValues();
