...
CREATE TABLE IF NOT EXISTS `#__profiles_people` (
  `profiles_person_id` SERIAL,
...
SELECT p.*, 
	...
	CONCAT(p.profiles_person_id, ':', p.alias) AS slug,
	CONCAT(d.profiles_department_id, ':', d.alias) AS department_slug,
	CONCAT(o.profiles_office_id, ':', o.alias) AS office_slug,
	u.name AS user_name,
	u.username AS user_username,
	u.email AS user_email,
	LEFT(p.lastname, 1) AS letter_name
FROM #__profiles_people AS p
LEFT JOIN #__profiles_departments AS d ON d.profiles_department_id = p.profiles_department_id
LEFT JOIN #__profiles_offices AS o ON o.profiles_office_id = p.profiles_office_id
LEFT JOIN #__users AS u ON u.id = p.user_id;


CREATE OR REPLACE VIEW #__profiles_view_departments AS 
SELECT d.*, 
	COUNT( DISTINCT p.profiles_person_id ) AS people,
	CONCAT(d.profiles_department_id, ':', d.alias) AS slug
FROM #__profiles_departments AS d
LEFT JOIN #__profiles_people AS p ON p.profiles_department_id = d.profiles_department_id AND p.enabled > 0
GROUP BY d.profiles_department_id;


CREATE OR REPLACE VIEW #__profiles_view_offices AS 
SELECT o.*, 
	COUNT( DISTINCT p.profiles_person_id ) AS people,
	CONCAT_WS('\n', address1, address2, CONCAT_WS(' ', city, state, postcode), country) AS address,
	CONCAT(o.profiles_office_id, ':', o.alias) AS slug
FROM #__profiles_offices AS o
LEFT JOIN #__profiles_people AS p ON p.profiles_office_id = o.profiles_office_id AND p.enabled > 0
...
INSERT IGNORE INTO `#__profiles_departments` (`profiles_department_id`, `title`, `alias`, `description`, `created_on`, `created_by`, `modified_on`, `modified_by`, `enabled`) VALUES
...
INSERT IGNORE INTO `#__profiles_people` (`profiles_person_id`, `profiles_department_id`, `profiles_office_id`, `firstname`, `middlename`, `lastname`, `alias`, `position`, `birthday`, `gender`, `mobile`, `email`, `bio`, `created_on`, `created_by`, `modified_on`, `modified_by`, `enabled`) VALUES
...
INSERT IGNORE INTO `#__profiles_offices` (`profiles_office_id`, `title`, `alias`, `description`, `address1`, `address2`, `city`, `state`, `postcode`, `country`, `phone`, `fax`, `coordinates`, `created_on`, `created_by`, `modified_on`, `modified_by`, `enabled`) VALUES
