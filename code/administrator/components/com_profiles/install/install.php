<?php
/**
 * @version		$Id$
 * @package		Profiles
 * @copyright	Copyright (C) 2009 - 2010 Nooku. All rights reserved.
 * @license 	GNU GPLv2 <http://www.gnu.org/licenses/old-licenses/gpl-2.0.html>
 * @link     	http://www.nooku.org
 */

// no direct access
defined('_JEXEC') or die('Restricted access');

jimport('joomla.filesystem.folder');
jimport('joomla.filesystem.file');

// Move the search plugin
$admin_path = 'administrator'.DS.'components'.DS.'com_profiles'.DS.'search';
$plugins_path = 'plugins'.DS.'search';
JFile::move($admin_path.DS.'profiles.xml',  $plugins_path.DS.'profiles.xml', JPATH_ROOT);
JFile::move($admin_path.DS.'profiles.php',  $plugins_path.DS.'profiles.php', JPATH_ROOT);

$status = new JObject();

// Insert in database
$row = JTable::getInstance('plugin');
$row->name = 'Search - Profiles';
$row->ordering = 1;
$row->folder = 'search';
$row->iscore = 0;
$row->access = 0;
$row->client_id = 0;
$row->element = 'profiles';
$row->published = 1;
$row->params = '';
if (!$row->store()) {
	// Install failed, roll back changes
	$this->parent->abort(JText::_('Plugin').' '.JText::_('Install').': '.$db->stderr(true));
	return false;
}
$status->set('search_plugin', true);

// Output status
?>
<h1>Business Enterprise Employee Repository (B.E.E.R.)</h1>
<script>$$('table.adminform')[0].getElementsByTagName('tr')[0].setStyle('display', 'none');</script>
<table class="adminlist">
	<thead>
		<tr>
			<th class="title"><?php echo JText::_('Task'); ?></th>
			<th width="60%"><?php echo JText::_('Status'); ?></th>
		</tr>
	</thead>
	<tfoot>
		<tr>
			<td colspan="2">&nbsp;</td>
		</tr>
	</tfoot>
	<tbody>
		<tr class="row1">
            <td class="key"><?php echo JText::_('Search Plugin'); ?></td>
            <td><?php echo ($status->get('search_plugin')) ? '<strong>'.JText::_('Installed').'</strong>' : '<em>'.JText::_('NOT Installed').'</em>'; ?></td>
        </tr>
		<tr class="row0">
			<td class="key"><?php echo JText::_('PHP Version'); ?></td>
			<td>
				<?php echo version_compare(phpversion(), '5.2', '>=')
					? '<strong>'.JText::_('OK').'</strong> - '.phpversion()
					: '<em>'.JText::_('You need at least PHP v5.2 to use B.E.E.R. You are using: ').phpversion().'</em>'; ?>
			</td>
		</tr>
		<tr class="row1">
			<td class="key"><?php echo JText::_('MySQL Version'); ?></td>
			<td>
				<?php echo version_compare($db->getVersion(), '5.0.41', '>=')
				? '<strong>'.JText::_('OK').'</strong> - '.$db->getVersion()
				: '<em>'.JText::_('You need at least MySQL v5.0.41 to use B.E.E.R. You are using: ').$db->getVersion().'</em>'; ?>
			</td>
		</tr>
	</tbody>
</table>
