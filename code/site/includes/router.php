...
class JRouterSite extends JRouter
{
    ...
    function _parseSefRoute(&$uri)
    {
        ...
        if (isset($vars['Itemid'])) {
            KService::get('application.pages')->setActive($vars['Itemid']);
        }
        ...
        if (!empty($route) && isset($this->_vars['option'])) {
            ...
            if ($page = KService::get('application.pages')->getActive()) {
                $vars = $page->link->query;
            }
        }
    ...
    function _buildSefRoute(&$uri)
    {
        ...
        if (isset($query['Itemid']) && !empty($query['Itemid'])) {
            $page = $pages->getPage($query['Itemid']);

            if (is_object($page) && $query['option'] == $page->link->query['option']) {
                $tmp = !empty($tmp) ? $page->route . '/' . $tmp : $page->route;
                $built = true;
            }
        }
    ...
    function &_createURI($url)
    {
        ...
        if (is_null($itemid)) {
            if ($option = $uri->getVar('option')) {
                $page = $pages->getPage($this->getVar('Itemid'));
                if (isset($page) && $page->link->query['option'] == $option) {
                    $uri->setVar('Itemid', $page->id);
                }
            } else {
            ...
            if (!$uri->getVar('option')) {
                $page = $pages->getPage($itemid);
                $uri->setVar('option', $page->link->query['option']);
            }
        }
