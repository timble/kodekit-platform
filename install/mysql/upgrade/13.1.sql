...
ALTER TABLE `#__articles` ADD `image` varchar(255) AFTER `description`;
...
UPDATE `#__pages` SET `link_url` = SUBSTRING(`link_url`, 11) WHERE `link_url` LIKE 'index.php?%';
...
VALUES
    ('menubar', 'Dashboard', 'dashboard', 'option=com_dashboard&view=dashboard', 'component', 1, (SELECT `extensions_component_id` FROM `#__extensions_components` WHERE `name` = 'com_dashboard'), 1, 0, 0);
...
VALUES
    ...
    ('menubar', 'Activity Logs', 'activity-logs', 'option=com_activities&view=activities', 'component', 1, (SELECT `extensions_component_id` FROM `#__extensions_components` WHERE `name` = 'com_activities'), 1, @base + 7, 1),
