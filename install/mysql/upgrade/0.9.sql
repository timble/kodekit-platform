...
UPDATE `modules` SET `comoonent` = 'users' WHERE `name` = 'mod_login';
UPDATE `modules` SET `component` = 'pages' WHERE `name` = 'mod_mainmenu';
UPDATE `modules` SET `component` = 'pages' WHERE `name` = 'mod_breadcrumbs';
UPDATE `modules` SET `component` = 'pages' WHERE `name` = 'mod_custom';
...
UPDATE `modules` SET `component` = 'articles' WHERE `name` = 'mod_articles';
...
ALTER TABLE `pages` CHANGE `componentid` `component` VARCHAR(50) NOT NULL;
...
ALTER TABLE `pages` ADD `created_by` INT(11) UNSIGNED AFTER `component`;
...
INSERT INTO `pages` (`menutype`, `title`, `slug`, `link_url`, `type`, `published`, `component`, `ordering`, `parent`, `sublevel`, `params`)
  VALUES
  ('menubar', 'Dashboard', 'dashboard', 'option=com_dashboard&view=dashboard', 'component', 1, 'dashboard', 1, 0, 0, '');

SET @base = LAST_INSERT_ID();

INSERT INTO `pages` (`menutype`, `title`, `slug`, `link_url`, `type`, `published`, `component`, `ordering`, `parent`, `sublevel`, `params`)
  VALUES
  ('menubar', 'Pages', 'pages', 'option=com_pages&view=pages', 'component', 1, 'pages', 2, 0, 0, ''),
  ('menubar', 'Content', 'content', NULL, 'separator', 1, NULL, 3, 0, 0, ''),
  ('menubar', 'Files', 'files', 'option=com_files&view=files', 'component', 1, 'files', 4, 0, 0, ''),
  ('menubar', 'Users', 'users', 'option=com_users&view=users', 'component', 1, 'users', 5, 0, 0, ''),
  ('menubar', 'Tools', 'tools', NULL, 'separator', 1, NULL, 7, 0, 0, ''),
  ('menubar', 'Activity Logs', 'activity-logs', 'option=com_activities&view=activities', 'component', 1, 'activities', 1, @base + 7, 1, ''),
  ('menubar', 'Articles', 'articles', 'option=com_articles&view=articles', 'component', 1, 'articles', 1, @base + 2, 1, ''),
  ('menubar', 'Contacts', 'contacts', 'option=com_contacts&view=contacts', 'component', 1, 7, 3, @base + 2, 1, ''),
  ('menubar', 'Languages', 'languages', 'option=com_languages&view=languages', 'component', 1, 'languages', 4, @base + 2, 1, ''),
  ('menubar', 'Articles', 'articles', 'option=com_articles&view=articles', 'component', 1, 'articles', 1, @base + 10, 2, ''),
  ('menubar', 'Categories', 'categories', 'option=com_articles&view=categories', 'component', 1, 'categories', 2, @base + 10, 2, ''),
  ('menubar', 'Languages', 'languages', 'option=com_languages&view=languages', 'component', 1, 'languages', 1, @base + 13, 2, ''),
  ('menubar', 'Pages', 'pages', 'option=com_pages&view=pages', 'component', 1, 'pages', 1, @base + 1, 1, ''),
  ('menubar', 'Menus', 'menus', 'option=com_pages&view=menus', 'component', 1, 'pages', 2, @base + 1, 1, ''),
  ('menubar', 'Modules', 'modules', 'option=com_pages&view=modules', 'component', 1, 'pages', 3, @base + 1, 1, ''),
  ('menubar', 'Users', 'users', 'option=com_users&view=users', 'component', 1, 'users', 1, @base + 4, 1, ''),
  ('menubar', 'Groups', 'groups', 'option=com_users&view=groups', 'component', 1, 'users', 2, @base + 4, 1, '');
...
ALTER TABLE `pages` ADD INDEX `ix_component` (`component`);
...
VALUES
    ('articles', 'articles', 'articles_article_id', 0),
    ('articles', 'categories', 'categories_category_id', 0);
