...
Library\ObjectManager::getInstance()->load('com:application.aliases');
$application =  Library\ObjectManager::getInstance()->get('com:application.dispatcher');
...
if($numbers['users'])
{
    ...
    $autoinc  =  Library\ObjectManager::getInstance()->get('com:users.database.table.users')->getSchema()->autoinc;

    for($i = 0; $i < $numbers['users']; $i++)
    {
        $row =  Library\ObjectManager::getInstance()->get('com:users.model.users')->getItem();
...
{
    $categories = array_fill_keys(array('articles'/*, 'contacts', 'weblinks'*/), array());
    ...
    if($categories)
    {
        ...
        foreach(array_keys($categories) as $table)
        {
            ...
            {
                $row =  Library\ObjectManager::getInstance()->get('com:categories.model.categories')->getItem();
...
if($numbers['articles'])
{
    ...
    {
        $row =  Library\ObjectManager::getInstance()->get('com:articles.model.articles')->getItem();
        $row->title = $generator->words(pick_random(range(1, 5)));
