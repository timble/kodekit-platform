...
class JHTMLGrid
{
	...
	function sort( $title, $order, $direction = 'asc', $selected = 0, $task=NULL )
	{
		...
		$html .= JText::_( $title );
		if ($order == $selected ) {
			$html .= JHTML::_('image.site',  $images[$index]);
		}
		$html .= '</a>';
		return $html;
	}
	...
	*/
	...
	{
		$image = JHTML::_('image.site',  $image, '/media/system/images/', NULL, NULL, JText::_( 'Save Order' ) );
	...
	function _checkedOut( &$row, $overlib = 1 )
	{
		...
		if ( $overlib )
		{
			...
			$checked_out = new KDate(array('date' => $row->checked_out_time));
			$date = $checked_out->format(JText::_('DATE_FORMAT_LC1'));
			$time = $checked_out->format('H:i');
