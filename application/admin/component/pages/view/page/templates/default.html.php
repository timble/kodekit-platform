...
<script>
    window.addEvent('domready', function(){
        ...
        new Pages.Page(<?= json_encode(array('active' => parameter('type')->component, 'type' => parameter('type')->name)) ?>);
    });
</script>
...
<?
    $query = array(
        'component' => parameter('type')->component,
        'view'      => parameter('type')->view
    );

    if(!empty(parameter('type')->layout) && parameter('layout') != 'default') {
        $query['layout'] = parameter('layout');
    }
?>

<form action="" method="post" class="-koowa-form" id="page-form">
    <input type="hidden" name="pages_menu_id" value="<?= parameter('menu') ?>" />
    <input type="hidden" name="type" value="<?= parameter('type')->name ?>" />
            ...
            <div id="components-inner">
                <? foreach($components as $component) : ?>
                    <? if(!empty($component->views)) : ?>
                        <a data-component="<?= $component->name ?>" class="component-<?= $component->name ?> <?= (parameter('type')->component == $component->name) ? 'active' : '' ?>" href="#"><span class="icon icon-16-component"></span><?= translate($component->title) ?></a>
                    <? endif ?>
                <? endforeach ?>
            </div>
        </div>
            ...
            <div data-component="<?= $component->name ?>" class="component-<?= $component->name ?>">
                <? foreach($component->views as $view) : ?>
                    ...
                    <div class="view">
                        ...
                        <a class="<?= (parameter('type')->view == $view->name && parameter('type')->layout == $layout->name) ? 'active' : '' ?>" href="<?= urldecode(route('menu='.$menu->id.'&type[component]='.$component->name.'&type[view]='.$view->name.'&type[layout]='.$layout->name.'&id='.$page->id)) ?>">
