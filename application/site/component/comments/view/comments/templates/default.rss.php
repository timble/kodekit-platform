...
<rss version="2.0"
    ...
    <channel>
        <title><?= translate('Comments for').' '.escape($row->title) ?></title>
        <description><![CDATA[<?= $row->introtext . $row->fulltext ?>]]></description>
        <link><?= route() ?></link>
        ...
        <? foreach($comments as $comment) : ?>
            <item>
                <title><?= translate('Comment on').' '.escape($row->title).' '.translate('by').' '.$comment->created_by_name ?></title>
                <link><?= helper('route.article', array('row' => $row)) ?> ?></link>
                <dc:creator><?= $comment->created_by_name ?></dc:creator>
                <guid isPermaLink="false"><?= helper('route.article', array('row' => $row)) ?> ?></guid>
